...
class ApiClient {
  ...
  async runBilling(
    asOfDate?: string,
  ): Promise<{ message: string; asOfDate: string; status: string }> {
    const params = asOfDate ? `?asOf=${asOfDate}` : "";
    return this.request<{ message: string; asOfDate: string; status: string }>(
      `/api/billing/run${params}`,
      {
        method: "POST",
      },
    );
  ...
  async getBillingJobs(): Promise<BillingJob[]> {
    const data = await this.request<{ jobs: BillingJob[] }>(
      "/api/billing/jobs",
    );
    return data.jobs || [];
  }
