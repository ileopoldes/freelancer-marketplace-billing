...
export async function billingRoutes(server: FastifyInstance, options: { prisma: PrismaClient }) {
  ...
  server.post('/api/billing/run', async (request, reply) => {
    try {
      ...
      server.log.info(`📅 [BILLING-API] Parsed asOfDate: ${asOfDate.toISOString()}`);
      ...
      setTimeout(async () => {
        try {
          server.log.info(`⚡ [BILLING-ENGINE] Starting execution for asOfDate: ${asOfDate.toISOString()}`);
          
          // Run the actual billing engine (which uses centralized job service)
          const result = await billingEngine.runBillingJob(asOfDate);
          
          server.log.info(`✅ [BILLING-ENGINE] Job completed successfully:`, result);
        } catch (error) {
