...
export class PayAsYouGoPricer {
    ...
    if (config.bulkDiscounts) {
      for (const discount of config.bulkDiscounts) {
        if (quantity >= discount.minQuantity) {
          const discountValue = baseAmount.amount
            .times(discount.discountPercentage)
            .div(100);
  ...
  private async getPricingConfig(
    ...
    return {
      ...
      tiers: [
        { limit: 100, price: createMoney("9.00") },
        { limit: 500, price: createMoney("8.50") },
        { limit: null, price: createMoney("8.00") },
      ],
    };
  }

  private applyTierPricing(
    tiers: MarketplaceEventTier[],
    quantity: number,
  ): number {
    let tierIndex = -1;
