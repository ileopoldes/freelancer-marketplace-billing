...
import { DiscountEngine, DiscountType, DiscountSchedule, DiscountContext } from '../src/services/pricing/DiscountEngine';
import { CreditManager } from '../src/services/billing/CreditManager';
import { ProrationEngine, PlanChange, MidCycleAdjustment } from '../src/services/pricing/ProrationEngine';
...
describe('Phase 5 Enhancements', () => {
  ...
  describe('CreditManager', () => {
    ...
    test('should apply credits in correct order (FIFO with type priority)', async () => {
      ...
      const totalApplied = result.applications.reduce((sum, app) => 
        sum + parseFloat(app.appliedAmount.amount.toString()), 0);
    ...
    test('should handle partial credit consumption', async () => {
      ...
      expect(result.applications[0].remainingAmount.amount.toString()).toBe('40');
    });
  ...
  describe('Enhanced InvoiceGenerator Integration', () => {
    ...
    test('should generate invoice with enhanced discounts and credits', async () => {
      ...
      const hasDiscount = lineItems.some(item => item.lineType === 'DISCOUNT');
